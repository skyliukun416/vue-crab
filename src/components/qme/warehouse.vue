<template>
    ...
    <div>
      <div class="common-title">仓库</div>
      <div class="category-articleWrap">
        <el-button type="primary" style="float:right;margin-bottom:10px" size="mini" icon="el-icon-plus" @click='createOrder()'>新建仓单</el-button>
...
export default {
    ...
    mounted(){
        ...
        this.orders = this.orders.filter(e=>{return e.value!=''})
    },
    methods: {
      
       pickOrder(index){
           this.selected = index;
           this.order[0] = this.orders[index];
       ...
       decline(){
          ...
          temp[this.index].approval = "已拒绝";
          
          localStorage.setItem('qmeOrders', JSON.stringify(temp));
       },

       createOrder(){
         this.$router.push({path: '/qme/home/form'})
       }
